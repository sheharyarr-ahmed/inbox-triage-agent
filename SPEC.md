...
- **Spend is still derived, never read.** No platform figure is trustworthy for this — `list_cost` moved units between two runs (D-047). Every balance number in this repo is priced from `session.usage` token counts at the pinned model's rates. The Console's own usage view is the independent cross-check.
  ...
  *Updated 2026-08-06.* **The cross-check has now been taken twice, and the bracket contained the true figure both times.** First: Console credits **$9.13** against a $10.38 baseline, so the platform observed **$1.25** where the derivation reported a **$1.82** ceiling. Second, after this session's three live runs: **$7.37**, so **$1.76** observed against a derived bracket of **$1.1167 .. $1.9393**.

  **The position within the bracket is not stable — 69% of ceiling the first time, 91% the second — and must not be predicted.** An inference drawn from the first reading, that the grader is billed near Haiku rates, is **withdrawn**: it rested on `list_cost`, which D-047 had already disqualified, and the second reading contradicts it. **Quote the ceiling, per A-1. Never budget on the floor, and never on a guessed position between them.** Had it ever come back *above* the ceiling, every budget stop in the build would be set too loose; it has not. **The derivation stays authoritative regardless**; the Console is a witness, not a new source. See D-069 and its correction.
